...
const bootstrapData = require('../data/bootstrap.json');
...
async function buildBootstrapState() {
    return normalizeState(bootstrapData || {});
}
...
function shouldRehydrateFromBootstrap(currentState, bootstrapState) {
    ...
    return (
        ...
        && currentState.users.length <= 1
        && currentState.motors.length <= 3
    ...
    if (useBlobStore()) {
        ...
        if (stored) {
            ...
            if (shouldRehydrateFromBootstrap(ensuredStoredState, bootstrapState)) {
                const rehydratedState = await ensureBaselineState(bootstrapState);
