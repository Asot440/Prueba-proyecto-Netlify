...
const projectRoot = process.cwd() || path.resolve(moduleDirectory, '..');
const bootstrapPathCandidates = [
    path.join(projectRoot, 'data', 'bootstrap.json'),
    path.join(moduleDirectory, '..', 'data', 'bootstrap.json'),
    path.join(moduleDirectory, '..', '..', 'data', 'bootstrap.json'),
    path.join(moduleDirectory, 'data', 'bootstrap.json')
];
const localStatePath = path.join(projectRoot, 'data', 'local-store.json');
...
async function resolveBootstrapPath() {
    for (const candidate of bootstrapPathCandidates) {
        try {
...
function shouldRehydrateFromBootstrap(currentState, bootstrapState) {
    if (!bootstrapState.motors.length) {
        return false;
    }

    return (
        currentState.inspections.length === 0
        && currentState.inspectionDetails.length === 0
        && (
            bootstrapState.motors.length > currentState.motors.length
            || bootstrapState.users.length > currentState.users.length
        )
    );
}

function mergeBootstrapState(currentState, bootstrapState) {
    const mergedState = normalizeState(currentState);
    const usersByUsername = new Map(
        mergedState.users.map((user) => [user.username.toLowerCase(), user])
    );
    const motorsByKey = new Map(
        mergedState.motors.map((motor) => [motor.equipment_key, motor])
    );

    for (const bootstrapUser of bootstrapState.users) {
        const key = bootstrapUser.username.toLowerCase();
        const existingUser = usersByUsername.get(key);

        if (existingUser) {
            existingUser.password_hash = existingUser.password_hash || bootstrapUser.password_hash;
            existingUser.role = existingUser.role || bootstrapUser.role;
            existingUser.permissions = Array.isArray(existingUser.permissions) && existingUser.permissions.length
                ? existingUser.permissions
                : bootstrapUser.permissions;
            continue;
        }

        mergedState.users.push({
            ...bootstrapUser,
            id: mergedState.meta.nextUserId++
        });
    }

    for (const bootstrapMotor of bootstrapState.motors) {
        if (motorsByKey.has(bootstrapMotor.equipment_key)) {
            continue;
        }

        mergedState.motors.push({
            ...bootstrapMotor,
            id: mergedState.meta.nextMotorId++
        });
    }
...
async function readState() {
    const bootstrapState = await buildBootstrapState();
    ...
    if (useBlobStore()) {
        ...
        if (stored) {
            const ensuredStoredState = await ensureBaselineState(stored);

            if (shouldRehydrateFromBootstrap(ensuredStoredState, bootstrapState)) {
                const rehydratedState = await ensureBaselineState(
                    mergeBootstrapState(ensuredStoredState, bootstrapState)
                );
                await store.setJSON(STATE_KEY, rehydratedState);
                return rehydratedState;
            }

            return ensuredStoredState;
        }

        const ensured = await ensureBaselineState(bootstrapState);
